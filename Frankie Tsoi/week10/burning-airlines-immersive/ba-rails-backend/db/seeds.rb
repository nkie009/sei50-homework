...
u1 = User.create! name: "Test User 1", email: 'on@gmail.com', password: 'chicken'
...
a2 = Airplane.create! name:'747 MAX', rows: 80, cols: 8
...
f1 = Flight.create! flight_number: 'BA256', departure_date: '2022-03-01 4:20', origin: 'SYD', destination: 'MEL', airplane_id: a1.id
...
raise Exception.new('Airplane flights assocation is broken') if Airplane.first.flights.length != 2
...
puts "Resevation #1 has user name #{Reservation.first.user.name} (should be 'Test User 1')"

puts "Resevation #2 has flight number #{Reservation.second.flight.flight_number} (should be 'BA256')"
